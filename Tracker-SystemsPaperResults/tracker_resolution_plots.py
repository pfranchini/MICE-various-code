...
def init_plots_data() :
  ...
  for tracker in [ 'upstream', 'downstream' ] :
    ...
    tracker_dict['pt_squared'] = ROOT.TH1F( tracker+'_pt_squared', \
                            'Transvere Momentum', 500, -0.0, 40000.0 )

    tracker_dict['pz'] = ROOT.TH1F( tracker+'_pz', \
                          'Longitudinal Momentum', 600, 0.0, 300.0 )
    ...
    tracker_dict['pt_residual_p'] = ROOT.TH2F( \
              tracker+'_pt_residual_p', "p_{t} Residuals in p", \
                                     PZ_BIN, PZ_MIN, PZ_MAX, 500, -50.0, 50.0 )
    ...
    tracker_dict['seed_p_residual_p'] = ROOT.TH2F( \
                               tracker+'_patrec_seed_p-p', "True p - Seed p", \
                                     PZ_BIN, PZ_MIN, PZ_MAX, 200, -50.0, 50.0 )
    tracker_dict['seed_pt_residual_p'] = ROOT.TH2F( \
                               tracker+'_patrec_seed_pt-p', "True p_{#perp} - Seed p", \
                                     PZ_BIN, PZ_MIN, PZ_MAX, 200, -50.0, 50.0 )
  ...
  for tracker in [ 'upstream', 'downstream' ] :
    ...
    missing_tracker['pt_squared'] = ROOT.TH1F(tracker+'_pt_sq_missing', \
                           "Missing Tracks pt", int(5*PT_BIN), 0.0, PT_MAX**2 )
  ...
  for tracker in ['upstream', 'downstream'] :
    ...
    data_dict['counters'][tracker]['chi2_ndf_cut'] = 0
...
def get_found_tracks(scifi_event, plot_dict, data_dict) :
  ...
  for track in tracks :
    ...
    if track.chi2() / track.ndf() < CHI2_NDF :
      data_dict['counters'][tracker]['chi2_ndf_cut'] += 1
      continue
...
#        plot_dict[tracker]['trackpoint_efficiency_pz'].Fill(False, virtual_pz)
      ...
      if reference_virt is not None :
        ...
        plot_dict['missing_tracks'][tracker]['pt_squared'].Fill( virtual_pt**2 )
...
def fill_plots(plot_dict, data_dict, hit_pairs) :
  ...
  for scifi_hit, virt_hit in hit_pairs :
    ...
    tracker_plots['pt_squared'].Fill(Pt_recon**2)
...
def fill_plots_seeds(plot_dict, data_dict, hit_pairs) :
  ...
  for scifi_track, virt_hit in hit_pairs :
    ...
    global counter
    counter += 1;
    #print str(counter)+": "+tracker+" P_res: "+str(P_res)
    ...
    tracker_plots['seed_pt_residual_p'].Fill(P_mc, Pt_res)
...
# Print out some simple stats
  ...
  print "Cut {0:0.0f} Upstream Tracks (Chi2/NDF Cut)".format( \
                             data_dict['counters']['upstream']['chi2_ndf_cut'] )
  print "Cut {0:0.0f} Downstream Tracks (P-Value Cut)".format( \
                           data_dict['counters']['downstream']['p_value_cut'] )
  print "Cut {0:0.0f} Downstream Tracks (Chi2/NDF Cut)".format( \
                          data_dict['counters']['downstream']['chi2_ndf_cut'] )
  print
...
#      for plot_axis in [ "residual_pt", "residual_pz", "residual_p" ] :
    for component in [ "pt_", "p_" ] :
      for plot_axis in [ "residual_pt", "residual_p" ] :
...
#      for plot_axis in [ "residual_pt", "residual_pz" ] :
    for plot_name in [ "pt_residual_pt", "pt_residual_pz", "pz_residual_pt", \
                                           "pz_residual_pz", "p_residual_p", "pt_residual_p" ] :
...
if __name__ == "__main__" : 
  ...
  parser.add_argument( '--cut_chi2_ndf', type=float, default=100.0, \
 help="Specify the Chi2/NDF below which tracks are removed from the analysis" )
...
#      'file for the reconstruction. I need the geometry information' )
  ...
  try :
    ...
    CHI2_NDF = namespace.cut_chi2_ndf
